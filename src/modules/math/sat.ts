import { abs, copy, reverse, scale, set, sqrt, sub } from "./math"
...
export function computePolygon(poly: IPolygon | number[], mat?: Float32Array): IPolygon | number[] {
    if (poly instanceof Array) {
        set(poly, poly[3], poly[4], poly[5])
        return v2m3(poly, mat)
    }
    const { p, c, e, n } = poly
    for (let i = 0; i < p.length; i += 2) {
        const vec = c[i / 2]
...
function pointInCircle(p: number[], c: number[]): boolean {
    const [x, y, r] = c
...
function pointInPolygon(p: number[], poly: IPolygon): boolean {
    copy(TEST_POINT, p)
...
function testPolygonPolygon(a: IPolygon, b: IPolygon, response?: IResponse): boolean {
    const aPoints = a.c
    const bPoints = b.c
...
function testCircleCircle(a: number[], b: number[], response?: IResponse): boolean {
    const [ax, ay, ar] = a
    const [bx, by, br] = b
...
function testPolygonCircle(polygon: IPolygon, circle: number[], response?: IResponse): boolean {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (overlapN && response && abs(overlap) < abs(response.o)) {
            response.o = overlap
...
function testCirclePolygon(circle: number[], polygon: IPolygon, response?: IResponse): boolean {
...
export function testPoint(p: number[], poly: IPolygon | number[]): boolean {
    return poly instanceof Array ? pointInCircle(p, poly) : pointInPolygon(p, poly)
}

export function testPoly(a: IPolygon | number[], b: IPolygon | number[], response?: IResponse): boolean {
    return a instanceof Array
        ? (b instanceof Array ? testCircleCircle(a, b, response) : testCirclePolygon(a, b, response))
        : (b instanceof Array ? testPolygonCircle(a, b, response) : testPolygonPolygon(a, b, response))
}
